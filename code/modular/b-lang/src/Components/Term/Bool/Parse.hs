...
-}
module Components.Term.Bool.Parse (
    parseTermInput
  ) where
...
-- |
parseTmFalse :: WithBoolTerm tm a
             => ParserHelperOutput
             -> Parser (tm a)           -- ^
parseTmFalse h =
  let
...
parseTermInput :: WithBoolTerm tm a
               => ParseTermInput tm a
parseTermInput =
  ParseTermInput
