...
data Term l n a =
    Var a
  | Lam Type (Scope (Name n ()) (Term l n) a)
  | App (Term l n a) (Term l n a)
  | Loc l (Term l n a) 
  | Add (Term l n a) (Term l n a)
  | Equ (Term l n a) (Term l n a)
  | And (Term l n a) (Term l n a)
...
lam_ :: Eq n => n -> Type -> Term l n n -> Term l n n
lam_ x t e = Lam t (abstract1Name x e)

whnf :: Term l n a -> Term l n a
whnf (App f x) = case whnf f of
  Lam _ f' -> whnf (instantiate1Name x f')
  f'     -> App f' x
whnf x = x

nf :: Term l n a -> Term l n a
nf (Lam t e) = Lam t . toScope . nf . fromScope $ e
nf (App f x) = case whnf f of
  Lam _ f' -> nf (instantiate1Name x f')
  f'     -> App (nf f') (nf x)
nf x = x
